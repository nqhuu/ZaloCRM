      ...
      <div v-else class="drawer-body">
        ...
        <section v-if="account.owner" class="d-section">
          <div class="h">
            ...
            <a v-if="account.canReassignOwner" class="link" @click="$emit('reassign-owner', account)">⚙ Chuyển nhượng</a>
          </div>
        ...
        <section class="d-section">
          ...
          <div v-for="c in account.crew" :key="c.accessId" class="assign-row">
            ...
            <span v-if="c.assignmentRole" class="role-badge assignment">
              {{ assignmentRoleLabel(c.assignmentRole) }}
            </span>
        ...
        <section class="d-section">
          <div class="danger-zone">
            ...
            <button
              v-if="account.canReassignOwner"
              class="dz-btn"
              @click="$emit('action', { accountId: account.id, action: 'delete' })"
            >
              Xoá nick khỏi CRM
            </button>
          </div>
...
function assignmentRoleLabel(role: string): string {
  ...
  if (secondaryMatch) return `Phụ ${secondaryMatch[1]}`;
  return role;
}
