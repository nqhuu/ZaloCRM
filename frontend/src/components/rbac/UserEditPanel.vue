      ...
      <aside class="panel">
        ...
        <div class="panel-body">
          ...
          <section v-if="canResetPassword" class="section">
            <h3 class="section-title">Mật khẩu đăng nhập</h3>
            <p class="hint-soft">
              Đặt lại mật khẩu tài khoản này về mật khẩu mặc định <strong>Abcd1234</strong>.
              Nhân viên nên đổi mật khẩu sau khi đăng nhập.
            </p>
            <button class="btn-primary-sm" :disabled="busy" @click="resetDefaultPassword">
...
const canResetPassword = computed(() => {
  if (!props.user || !['owner', 'admin'].includes(props.currentUserRole)) return false;
  return props.currentUserRole === 'owner' || props.user.role !== 'owner';
});
...
async function resetDefaultPassword() {
  if (!props.user || !canResetPassword.value) return;
  const label = props.user.fullName || props.user.email;
  if (!confirm(`Đặt lại mật khẩu của "${label}" về Abcd1234?`)) return;
  busy.value = true;
  error.value = '';
  try {
    await api.post(`/users/${props.user.id}/reset-default-password`);
    alert(`Đã đặt lại mật khẩu của ${label} về Abcd1234.`);
  } catch (e: any) {
    error.value = e?.response?.data?.error || 'Lỗi đặt lại mật khẩu';
  } finally {
    busy.value = false;
  }
}
