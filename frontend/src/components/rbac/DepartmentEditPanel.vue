    ...
    <div v-if="open" class="panel-backdrop" @click.self="$emit('close')">
      <aside class="panel">
        ...
        <div class="panel-body">
          ...
          <section class="section">
            <h3 class="section-title">Cấu hình Hồ sơ lưu trữ</h3>
            <label class="field-label">Loại hồ sơ mặc định</label>
            <select
              v-model="defaultArchiveRecordType"
              class="field-input"
              :disabled="busy"
              @change="saveArchiveDefaults"
            >
            ...
            <p class="parent-hint">
              Tự động áp dụng khi nhân viên phòng này tạo hồ sơ mới. Người có quyền vẫn có thể chọn lại.
            </p>
          </section>
...
const defaultArchiveRecordType = ref('order');
...
watch(() => [props.open, props.node?.id], async () => {
  ...
  defaultArchiveRecordType.value = props.node.defaultArchiveRecordType || 'order';
  error.value = '';
...
async function saveArchiveDefaults() {
  if (!props.node) return;
  busy.value = true;
  error.value = '';
  try {
    await api.patch(`/departments/${props.node.id}`, {
      defaultArchiveRecordType: defaultArchiveRecordType.value,
    });
    await store.loadDepartments();
  } catch (e: any) {
    error.value = e?.response?.data?.error || 'Lỗi lưu cấu hình hồ sơ';
    defaultArchiveRecordType.value = props.node.defaultArchiveRecordType || 'order';
  } finally {
    busy.value = false;
  }
