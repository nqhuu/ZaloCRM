      ...
      >
        ...
        <template v-else>
          <button
            ...
            @click.stop="emit('open-reply', reply)"
          >
          ...
          <div v-else-if="getImageUrl(message)">
            <div class="media-download-wrap">
              ...
              <button
                ...
                title="Tải xuống"
                @click.stop="downloadCurrentMedia('image')"
              >
          ...
          <div v-else-if="message.contentType === 'sticker'">
            <div class="sticker-msg media-download-wrap">
              ...
              <button
                v-if="stickerDownloadUrl"
                type="button"
                class="media-download-btn"
                title="Tải xuống"
                @click.stop="downloadCurrentMedia('sticker')"
              >
                <v-icon size="16">mdi-download</v-icon>
              </button>
            </div>
          ...
          <div v-else-if="message.contentType === 'video'">
            <div class="video-msg">
              <div v-if="videoThumb" class="video-thumb-wrap" @click="openVideo">
                ...
                <button
                  v-if="getVideoUrl(message)"
                  type="button"
                  class="media-download-btn"
                  title="Tải xuống"
                  @click.stop="downloadCurrentMedia('video')"
                >
                  <v-icon size="16">mdi-download</v-icon>
                </button>
              </div>
          ...
          <div v-else-if="message.contentType === 'voice' || message.contentType === 'audio'">
            <div class="voice-msg-v2">
              ...
              <button
                ...
                title="Tải xuống"
                @click.stop="downloadCurrentMedia(message.contentType)"
              >
                <v-icon size="16">mdi-download</v-icon>
              </button>
          ...
          <div v-else-if="message.contentType === 'gif'">
            <div class="gif-msg media-download-wrap">
              ...
              <button
                ...
                class="media-download-btn"
                title="Tải xuống"
                @click.stop="downloadCurrentMedia('gif')"
              >
                <v-icon size="16">mdi-download</v-icon>
              </button>
            </div>
        ...
        <div class="bubble-meta" :class="{ 'bubble-meta-self': isSelf }">
          <v-menu
            v-if="archiveIndicator"
            location="top"
            offset="8"
            content-class="message-archive-popover-shell"
          >
            <template #activator="{ props: menuProps }">
              <button
                type="button"
                class="archive-indicator-chip"
                v-bind="menuProps"
                @click.stop
              >
                <v-icon size="13">mdi-folder-star-outline</v-icon>
                <span>{{ archiveIndicator.visibleCount }}</span>
              </button>
            </template>
            <div class="archive-indicator-popover" @click.stop>
              <div class="archive-indicator-title">Tin nhắn đã lưu trong hồ sơ</div>
              <button
                v-for="story in archiveIndicator.stories"
                :key="story.id"
                type="button"
                class="archive-indicator-story"
                @click.stop="openArchiveStory(story.id)"
              >
                <span class="archive-indicator-story-name">{{ story.title }}</span>
                <span v-if="story.statusName" class="archive-indicator-story-status">{{ story.statusName }}</span>
              </button>
              <div
                v-if="archiveIndicator.visibleCount > archiveIndicator.stories.length"
                class="archive-indicator-more"
              >
                +{{ archiveIndicator.visibleCount - archiveIndicator.stories.length }} hồ sơ khác
              </div>
            </div>
          </v-menu>
...
const emit = defineEmits<{
  ...
  'open-reply': [reply: NonNullable<Message['reply']>];
}>();

const router = useRouter();
const archiveIndicator = computed(() => {
  const info = props.message.archiveInfo;
  return info && info.visibleCount > 0 ? info : null;
});
...
function openArchiveStory(storyId: string) {
  void router.push({ path: '/archive', query: { storyId } });
}
...
function getImageUrl(msg: Message): string | null {
  ...
  if (msg.contentType === 'image' && msg.content) {
    ...
    try {
      ...
      const rawUrl = p.hdUrl || p.href || p.normalUrl || p.thumbUrl || p.thumb || null;
  ...
  if (msg.content?.startsWith('{')) {
    try {
      ...
      if (href && (href.includes('zdn.vn') || href.includes('zaloapp.com') || href.includes('zalocontent.com'))) {
        ...
        if (!fileExt || /^(jpg|jpeg|png|webp|gif)$/i.test(fileExt)) return resolveAttachmentUrl(href);
      }
    } catch {}
  }
  return null;
...
function getVideoUrl(msg: Message): string | null {
  ...
  try {
    const p = JSON.parse(msg.content);
    const rawUrl = p.href || p.fileUrl || p.normalUrl || null;
    return rawUrl ? resolveAttachmentUrl(rawUrl) : null;
  } catch { return null; }
}
...
function getFileInfo(msg: Message): { name: string; size: string; href: string } | null {
  ...
  try {
    ...
    if (p.href && p.name && typeof p.size === 'number' && p.mime && !p.mime.startsWith('image/') && !p.mime.startsWith('video/')) {
      ...
      return { name: p.name, size, href: String(p.href) };
    }
    ...
    if (params?.fileExt || params?.fType === 1) {
      ...
      return { name: p.title || `file.${params.fileExt || 'unknown'}`, size, href: String(p.href || '') };
    }
  } catch {}
  return null;
}

function mediaFileName(kind: string, msg: Message = props.message) {
  const p = safeParse(msg.content);
  const params = typeof p?.params === 'string' ? safeParse(p.params) : (p?.params as Record<string, unknown> | undefined);
  const ext = typeof params?.fileExt === 'string' && params.fileExt.trim()
    ? params.fileExt.trim().replace(/^\./, '')
    : ({
        image: 'jpg',
        video: 'mp4',
        voice: 'mp3',
        ...
        gif: 'gif',
        sticker: 'png',
      } as Record<string, string>)[kind] || 'bin';
  const explicit = [p?.fileName, p?.filename, p?.name, p?.title]
    .find((value) => typeof value === 'string' && value.trim() && !/^https?:\/\//i.test(value.trim())) as string | undefined;
  if (explicit) {
    const clean = explicit.trim().replace(/[\\/:*?"<>|]/g, '_');
    return /\.[a-z0-9]{2,8}$/i.test(clean) ? clean : `${clean}.${ext}`;
  }
  return `zalo-${kind || 'attachment'}-${msg.id}.${ext}`;
}
...
const stickerDownloadUrl = computed(() => stickerMeta.value?.staticUrl || stickerMeta.value?.spriteUrl || stickerFallbackUrl.value);
...
function extractMediaUrl(_kind: string, content: string | null): string | null {
  if (!content) return null;
  if (content.startsWith('http')) return resolveAttachmentUrl(content);
  const p = safeParse(content);
  if (!p) return null;
  const url = (p.hdUrl as string) || (p.href as string) || (p.url as string) || (p.normalUrl as string) || '';
  return typeof url === 'string' && url.startsWith('http') ? resolveAttachmentUrl(url) : null;
}
...
function openVideo() {
  ...
  if (typeof url === 'string' && url.startsWith('http')) {
    emit('preview-video', resolveAttachmentUrl(url));
  }
}

function currentMediaUrl(kind: string) {
  if (kind === 'image') return getImageUrl(props.message);
  if (kind === 'video') return getVideoUrl(props.message);
  if (kind === 'voice' || kind === 'audio') return voiceUrl.value;
  if (kind === 'gif') return gifUrl.value;
  if (kind === 'sticker') return stickerDownloadUrl.value;
  return null;
}

function downloadCurrentMedia(kind: string) {
  downloadAttachment(currentMediaUrl(kind), mediaFileName(kind));
}
...
function openFile(href: string, fileName?: string) {
...
.archive-indicator-chip:hover {
  background: rgba(37, 99, 235, 0.12);
  border-color: rgba(37, 99, 235, 0.24);
  transform: translateY(-1px);
}
.archive-indicator-popover {
  min-width: 230px;
  max-width: 280px;
  padding: 10px;
  border-radius: 14px;
...
.archive-indicator-story:hover {
  background: #eff6ff;
}
.archive-indicator-story-name {
  font-size: 12.5px;
  font-weight: 700;
  color: #1e3a8a;
}
.archive-indicator-story-status {
  font-size: 11px;
  color: #64748b;
}
.archive-indicator-more {
  margin-top: 8px;
  padding-top: 8px;
...
.media-download-wrap:hover .media-download-btn,
.video-thumb-wrap:hover .media-download-btn,
.chat-video-wrap:hover .media-download-btn {
  opacity: 1;
}
