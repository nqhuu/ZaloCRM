<template>
  <div class="dept-page">
    <header class="page-hero">
      ...
      <button v-if="authStore.isAdmin" class="create-user-btn" @click="openCreateUser">
    ...
    <v-dialog v-model="createDialog" max-width="620">
      <v-card rounded="xl">
        ...
        <v-card-subtitle>Tạo tài khoản và phân quyền ban đầu trong một bước.</v-card-subtitle>
        <v-card-text>
          <div class="create-user-grid">
            ...
            />
            <v-select
              v-model="createForm.permissionGroupId"
              :items="flatGroups"
              item-title="name"
              item-value="id"
              clearable
          ...
          <v-alert v-if="createError" type="error" density="compact" variant="tonal">
            {{ createError }}
          </v-alert>
        </v-card-text>
        <v-card-actions>
          ...
          <v-btn color="primary" :loading="creatingUser" @click="createUser">Tạo tài khoản</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </div>
...
const createForm = ref({
  ...
  role: 'member',
  departmentId: '',
  deptRole: 'member' as 'leader' | 'deputy' | 'member',
  permissionGroupId: '',
});
...
const createRoleOptions = computed(() => [
  { title: 'Nhân viên', value: 'member' },
  ...(authStore.isOwner ? [{ title: 'Quản trị viên', value: 'admin' }] : []),
]);
const departmentRoleOptions = [
  { title: 'Nhân viên', value: 'member' },
  { title: 'Phó phòng', value: 'deputy' },
  { title: 'Trưởng phòng', value: 'leader' },
];
...
async function createUser() {
  ...
  if (!form.fullName.trim() || !form.email.trim() || !form.password) {
    createError.value = 'Họ tên, email và mật khẩu là bắt buộc.';
    return;
  }
  ...
  try {
    ...
    if (form.departmentId) {
      try {
        await api.post(`/departments/${form.departmentId}/members`, {
          userId: user.id,
          deptRole: form.deptRole,
        });
      } catch {
        setupErrors.push('phòng ban');
      }
    }
    if (form.permissionGroupId) {
      try {
        await api.patch(`/rbac/users/${user.id}/permission-group`, {
          permissionGroupId: form.permissionGroupId,
        });
      } catch {
        setupErrors.push('nhóm quyền');
      }
    }
    await Promise.all([store.loadUsers(), store.loadDepartments(), store.loadPermissionGroups()]);
    createDialog.value = false;
    if (setupErrors.length) {
      toast.warning(`Đã tạo ${user.fullName}, nhưng chưa gán được ${setupErrors.join(' và ')}.`);
    } else {
      toast.success(`Đã tạo tài khoản ${user.fullName}`);
    }
  } catch (error: any) {
    createError.value = error?.response?.data?.error || 'Không thể tạo tài khoản.';
  } finally {
...
.create-user-btn:hover { background: #2d3440; }
.create-user-grid {
  display: grid;
  grid-template-columns: 1fr 1fr;
  gap: 4px 12px;
}
.create-user-group { grid-column: 1 / -1; }
...
@media (max-width: 640px) {
  ...
  .create-user-group { grid-column: auto; }
}
</style>
