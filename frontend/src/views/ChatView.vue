<template>
  ...
  <div v-else class="smax-chat-grid">
    ...
    <MessageThread
      ...
      :focus-message-id="activeFocusMessageId"
      :focus-message-token="activeFocusMessageToken"
      class="smax-msg-col"
      @send="sendMessage"
      ...
      @focus-reply-message="focusReplyMessage"
    />
...
import { useChat, type ReplyMessageRef } from '@/composables/use-chat';
...
const {
  ...
  fetchConversations, fetchAiConfig, fetchMessages, fetchMessagesAround, selectConversation, sendMessage,
...
function onSelectConv(convId: string) {
  manualFocusMessageId.value = '';
  manualFocusMessageToken.value = '';
...
let lastFocusedMessageKey = '';
let originFocusRequestId = 0;

function routeMessageId(): string {
  return typeof route.query.messageId === 'string' ? route.query.messageId : '';
}

function routeMessageFocusToken(): string {
  return typeof route.query.focusAt === 'string' ? route.query.focusAt : '';
}

const manualFocusMessageId = ref('');
const manualFocusMessageToken = ref('');
const activeFocusMessageId = computed(() => manualFocusMessageId.value || routeMessageId());
const activeFocusMessageToken = computed(() => manualFocusMessageToken.value || routeMessageFocusToken());

function findMessageByAnyId(messageId: string) {
  if (!messageId) return undefined;
  return messages.value.find((message) =>
    message.id === messageId
    || message.zaloMsgId === messageId
    || message.zaloMsgIdNum === messageId,
  );
}
...
async function waitForMessageElement(messageId: string, attempts = 8) {
...
async function scrollToRouteMessage(force = false) {
  const convId = selectedConvId.value;
  const messageId = routeMessageId();
  if (!convId || !messageId) return;
  const focusKey = `${convId}:${messageId}:${routeMessageFocusToken()}`;
  let targetMessageId = findMessageByAnyId(messageId)?.id || messageId;
  if (!force && lastFocusedMessageKey === focusKey && findMessageElement(targetMessageId)) return;
  const requestId = ++originFocusRequestId;

  let element = await waitForMessageElement(targetMessageId, 4);
  if (!element) {
    const alreadyLoaded = Boolean(findMessageByAnyId(messageId));
    if (!alreadyLoaded) {
      const around = await fetchMessagesAround(convId, messageId);
      if (requestId !== originFocusRequestId) return;
      if (!around.found) {
        toast.error('Không tìm thấy tin nhắn gốc trong hội thoại');
        return;
      }
      targetMessageId = around.targetMessageId || messageId;
    }
    targetMessageId = findMessageByAnyId(targetMessageId)?.id || findMessageByAnyId(messageId)?.id || targetMessageId;
    element = await waitForMessageElement(targetMessageId, 12);
  }
  if (!element || requestId !== originFocusRequestId) return;

  lastFocusedMessageKey = focusKey;
...
function replyTargetCandidates(reply: ReplyMessageRef) {
  return [reply.sourceMessageId, reply.msgId, reply.cliMsgId]
    .filter((value): value is string => Boolean(value && String(value).trim()));
}

function resolveLoadedReplyTarget(reply: ReplyMessageRef) {
  for (const candidate of replyTargetCandidates(reply)) {
    const message = findMessageByAnyId(candidate);
    if (message) return message.id;
  }
  return '';
}

async function focusReplyMessage(reply: ReplyMessageRef) {
  const convId = selectedConvId.value;
  if (!convId) return;
  const loadedTargetId = resolveLoadedReplyTarget(reply);
  if (loadedTargetId) {
    manualFocusMessageId.value = loadedTargetId;
    manualFocusMessageToken.value = String(Date.now());
    return;
  }

  const lookupId = replyTargetCandidates(reply)[0];
  if (!lookupId) {
    toast.error('Không xác định được tin nhắn được trả lời');
    return;
  }

  const around = await fetchMessagesAround(convId, lookupId);
  if (!around.found) {
    toast.error('Không tìm thấy tin nhắn được trả lời hoặc bạn không có quyền xem');
    return;
  }
  manualFocusMessageId.value = around.targetMessageId || lookupId;
  manualFocusMessageToken.value = String(Date.now());
}
...
watch(
  ...
  (id) => {
    ...
    } else if (typeof id === 'string' && id && routeMessageId()) {
      void scrollToRouteMessage(true);
    }
  },
  { immediate: false },
);
...
watch(
  [selectedConvId, () => route.query.messageId, () => route.query.focusAt, () => messages.value.length],
  () => {
    if (routeMessageId()) void scrollToRouteMessage();
  },
);
...
:global(.msg-origin-highlight) {
  border-radius: 18px;
  animation: origin-message-pulse 3.2s ease;
}

@keyframes origin-message-pulse {
  0%, 100% {
    box-shadow: none;
    background: transparent;
  }
  12%, 72% {
    box-shadow: 0 0 0 3px rgba(239, 68, 68, .32), 0 12px 30px rgba(249, 115, 22, .24);
