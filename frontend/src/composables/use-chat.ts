...
export interface MessageArchiveStoryRef {
  id: string;
  title: string;
...
export interface MessageArchiveInfo {
  visibleCount: number;
  stories: MessageArchiveStoryRef[];
}
...
export interface Message {
  ...
  seenAt?: string | null;
  archiveInfo?: MessageArchiveInfo | null;
}
...
export function useChat() {
  ...
  async function fetchMessagesAround(convId: string, messageId: string) {
    if (messagesConvId.value !== convId) {
      messages.value = [];
      messagesConvId.value = convId;
    }
    loadingMsgs.value = true;
    try {
      const res = await api.get(`/conversations/${convId}/messages/around/${messageId}`, {
        params: { before: 50, after: 50 },
      });
      const list = (res.data.messages as RawMessage[]).map(normalizeMessage);
      if (selectedConvId.value === convId && messagesConvId.value === convId) {
        messages.value = list;
      ...
      const targetMessageId = res.data.targetMessageId || messageId;
      return {
        ok: true,
        found: list.some((message) => message.id === targetMessageId),
        targetMessageId,
      };
    } catch (err) {
      console.error('Failed to fetch messages around:', err);
      return { ok: false, found: false, targetMessageId: messageId };
    } finally {
      if (selectedConvId.value === convId) loadingMsgs.value = false;
    }
  }
  ...
  function initSocket() {
    ...
    socket.on('archive:message-recalled', (data: {
      ...
      if (data.assignedUserId && data.assignedUserId !== authStore.user?.id && !['owner', 'admin'].includes(authStore.user?.role || '')) return;
      toast.warning(`${data.senderName} đã thu hồi trong hồ sơ "${data.storyTitle || data.conversationName}": ${data.content}`, 8000);
    });
    socket.on('archive:backup-completed', (data: { userId?: string; summary: string }) => {
      if (!data.userId || data.userId === authStore.user?.id) toast.success(`Đã backup Google: ${data.summary}`, 5000);
    });
    socket.on('archive:backup-failed', (data: { userId?: string; error: string }) => {
      if (!data.userId || data.userId === authStore.user?.id) toast.error(`Backup Google thất bại: ${data.error}`, 7000);
    });
    socket.on('archive:end-of-day-reminder', (data: { userId?: string; count: number }) => {
      if (data.userId === authStore.user?.id || ['owner', 'admin'].includes(authStore.user?.role || '')) {
        toast.warning(`Cuối ngày còn ${data.count} nội dung chưa hoàn thành`, 8000);
      }
    });
