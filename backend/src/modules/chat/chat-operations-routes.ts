...
import { processArchivedMessageRecall } from '../archive/archive-service.js';
...
async function resolveMessageRefs(conversationId: string, messageId: string, userOrgId: string): Promise<ResolvedMessageRefs | null> {
  const message = await prisma.message.findFirst({
    ...
    select: {
      ...
      repliedByUserId: true, content: true, contentType: true, sentAt: true, attachments: true,
    },
  });
...
const FORWARD_MEDIA_TYPES = new Set(['image', 'video', 'voice', 'audio', 'file', 'gif']);
const MEDIA_URL_FIELDS: Record<string, string[]> = {
  ...
  gif: ['hdUrl', 'href', 'normalUrl', 'url', 'fileUrl', 'thumbUrl', 'thumb', 'thumbnail'],
  file: ['href', 'fileUrl', 'url', 'downloadUrl', 'rawUrl'],
};
...
function objectCandidates(value: unknown, depth = 0): Record<string, unknown>[] {
  const parsed = safeJsonValue(value);
  if (!parsed || depth > 3) return [];
  if (Array.isArray(parsed)) return parsed.flatMap((item) => objectCandidates(item, depth + 1));
  if (typeof parsed !== 'object') return [];
  const object = parsed as Record<string, unknown>;
  return [
    object,
    ...objectCandidates(object.params, depth + 1),
    ...objectCandidates(object.attachment, depth + 1),
    ...objectCandidates(object.attachments, depth + 1),
    ...objectCandidates(object.media, depth + 1),
    ...objectCandidates(object.file, depth + 1),
  ];
}

function pickForwardMedia(
  content: string | null,
  contentType: string,
  attachments?: unknown,
): { url: string; filename?: string; mimeType?: string } | null {
  if (content && /^https?:\/\//i.test(content.trim())) return { url: content.trim() };
  const fields = MEDIA_URL_FIELDS[contentType] ?? ['href', 'url', 'fileUrl'];
  const candidates = [
    ...objectCandidates(content),
    ...objectCandidates(attachments),
  ];
  for (const candidate of candidates) {
    for (const field of fields) {
      ...
      if (typeof value === 'string' && /^https?:\/\//i.test(value)) {
        return {
          url: value,
          filename: pickString(candidate, ['fileName', 'filename', 'name', 'title']),
          mimeType: pickString(candidate, ['mime', 'mimeType', 'contentType', 'type']),
        };
      }
    }
  }
...
export async function chatOperationsRoutes(app: FastifyInstance) {
  ...
  app.delete('/api/v1/conversations/:id/messages/:msgId', chatAccess, async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      const io = (app as any).io as Server;
      if (!onlyMe) await processArchivedMessageRecall([refs.messageId], io);
  ...
  app.post('/api/v1/conversations/:id/messages/:msgId/undo', chatAccess, async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      const io = (app as any).io as Server;
      await processArchivedMessageRecall([refs.messageId], io);
      io?.emit('chat:deleted', { conversationId: id, messageId: refs.messageId, zaloMsgId: refs.zaloMsgId });
      return { success: true };
    } catch (err) { return handleError(err, reply); }
  });
  ...
  app.post('/api/v1/conversations/:id/forward', chatAccess, async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      if (['text', 'rich'].includes(refs.contentType)) {
        ...
        const media = pickForwardMedia(refs.content, refs.contentType, refs.attachments);
        ...
        return reply.status(400).send({
          error: 'Chưa hỗ trợ chuyển tiếp loại tin nhắn này.',
        });
      }
