...
import { actorHasArchiveGrant, archiveScopeWhere, type ArchiveActor } from '../archive/archive-access.js';
...
async function buildMessageArchiveInfo(
  actor: ArchiveActor,
  messageIds: string[],
): Promise<Map<string, MessageArchiveInfo>> {
  const result = new Map<string, MessageArchiveInfo>();
  if (messageIds.length === 0) return result;
  if (!(await actorHasArchiveGrant(actor, 'access'))) return result;

  const scope = await archiveScopeWhere(actor);
  const stories = await prisma.archiveStory.findMany({
    where: {
      ...scope,
      messages: { some: { sourceMessageId: { in: messageIds } } },
    },
    select: {
      id: true,
      title: true,
      ...
      updatedAt: true,
      businessStatus: true,
      statusDefinition: { select: { name: true, colorToken: true } },
      messages: {
  ...
  for (const story of stories) {
    const storyRef = {
      ...
      statusName: story.statusDefinition?.name || story.businessStatus || null,
      statusColorToken: story.statusDefinition?.colorToken || null,
    };
    for (const message of story.messages) {
      const current = result.get(message.sourceMessageId) ?? {
        visibleCount: 0,
        stories: [],
      };
      current.visibleCount += 1;
      if (current.stories.length < 5) current.stories.push(storyRef);
      result.set(message.sourceMessageId, current);
    }
  }
...
async function serializeMessagesForResponse(input: {
  actor: ArchiveActor;
  request: FastifyRequest;
  conversation: { zaloAccount: { privacyMode: string; ownerUserId: string | null } };
  messages: any[];
}) {
  const { buildPrivacyContext, redactMessage } = await import('../privacy/redact.js');
  const privacyCtx = await buildPrivacyContext(input.request);
  const archiveInfoByMessageId = await buildMessageArchiveInfo(
    input.actor,
    input.messages.map((message) => message.id),
  );

  return input.messages.map((message) => {
    const redacted = redactMessage(message as any, input.conversation as any, privacyCtx);
    const archiveInfo = archiveInfoByMessageId.get(message.id);
    return {
      ...redacted,
      zaloMsgIdNum: (redacted as any).zaloMsgIdNum?.toString() ?? null,
      ...(archiveInfo ? { archiveInfo } : {}),
    };
  });
}

function beforeTargetWhere(target: { zaloMsgIdNum: bigint | null; sentAt: Date }) {
  return target.zaloMsgIdNum
    ? { zaloMsgIdNum: { lt: target.zaloMsgIdNum } }
    : { sentAt: { lt: target.sentAt } };
}

function afterTargetWhere(target: { zaloMsgIdNum: bigint | null; sentAt: Date }) {
  return target.zaloMsgIdNum
    ? { zaloMsgIdNum: { gt: target.zaloMsgIdNum } }
    : { sentAt: { gt: target.sentAt } };
}
...
function scopedAccountFilter(accessibleIds: string[], requestedIds: string[]) {
  const allowed = requestedIds.length
    ? requestedIds.filter((id) => accessibleIds.includes(id))
    : accessibleIds;
  if (allowed.length === 0) return 'EMPTY_ACCOUNT_SCOPE_NO_MATCH';
...
export async function chatRoutes(app: FastifyInstance) {
  ...
  app.get('/api/v1/conversations/counts', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    baseWhere.zaloAccountId = scopedAccountFilter(scope.accessibleIds, accountId ? [accountId] : []);
  ...
  app.get('/api/v1/conversations', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    where.zaloAccountId = folderAccountIds !== null && folderAccountIds.length === 0
      ? 'EMPTY_FOLDER_NO_MATCH'
      : scopedAccountFilter(scope.accessibleIds, accountIdList);
  ...
  app.get('/api/v1/conversations/:id/messages/around/:messageId', {
    ...
    const { before = '50', after = '50' } = request.query as QueryParams;
    const beforeLimit = Math.min(Math.max(parseInt(before, 10) || 50, 0), 100);
    const afterLimit = Math.min(Math.max(parseInt(after, 10) || 50, 0), 100);
    ...
    const numericMessageId = /^\d+$/.test(messageId) ? BigInt(messageId) : null;
    const target = await prisma.message.findFirst({
      where: {
        ...
        OR: [
          { id: messageId },
          { zaloMsgId: messageId },
          ...(numericMessageId ? [{ zaloMsgIdNum: numericMessageId }] : []),
        ],
      },
    ...
    if (!target) {
      return reply.status(404).send({
        error: 'Tin nhắn gốc không còn tồn tại trong hệ thống',
        code: 'SOURCE_MESSAGE_NOT_FOUND',
      });
    }

    const beforeWhere = { conversationId: id, ...beforeTargetWhere(target) };
    const afterWhere = { conversationId: id, ...afterTargetWhere(target) };
    const [olderDesc, newerAsc, beforeCount, afterCount] = await Promise.all([
      ...
      prisma.message.count({ where: beforeWhere }),
      prisma.message.count({ where: afterWhere }),
    ]);

    const ordered = [...olderDesc.reverse(), target, ...newerAsc];
    const messages = await serializeMessagesForResponse({
      actor: user as ArchiveActor,
      request,
      conversation,
      messages: ordered,
    });
    ...
    return {
      ...
      hasBefore: beforeCount > beforeLimit,
      hasAfter: afterCount > afterLimit,
    };
  });
  ...
  app.get('/api/v1/conversations/:id/messages', {
    ...
    const redacted = await serializeMessagesForResponse({
      actor: user as ArchiveActor,
      request,
      conversation,
      messages: ordered,
  ...
  app.post('/api/v1/conversations/:id/messages', { preHandler: requireZaloAccess('chat') }, async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      if (replyMessageId) {
        try {
          archiveSync = await syncReplyMessageToArchive({
            messageId: message.id,
            replyToMessageId: replyMessageId,
            actor: { id: user.id, orgId: user.orgId, role: user.role },
            io,
          });
        } catch (archiveError) {
          logger.error('[chat] Message sent but archive reply sync failed:', archiveError);
          archiveSync = { status: 'failed' };
        }
      }

      return { ...safeMessage, _archiveSync: archiveSync };
    } catch (err) {
