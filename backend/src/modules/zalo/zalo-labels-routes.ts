...
export async function syncLabelsForAccount(
  ...
  if (!isDelta) {
    ...
    const fallbackGroupName = `${groupName} [${accountId.slice(0, 8)}]`;
    const groupNameOwner = await prisma.crmTagGroup.findFirst({
      where: {
        orgId,
        name: groupName,
        OR: [
          { zaloAccountId: null },
          { zaloAccountId: { not: accountId } },
        ],
      },
      select: { id: true },
    });
    const desiredGroupName = groupNameOwner ? fallbackGroupName : groupName;
    if (groupNameOwner) {
      logger.warn(`[zalo-labels] group name "${groupName}" already exists; using "${desiredGroupName}" for account ${accountId}`);
    }
