...
export async function syncFriendsForAccount(
  ...
  try {
    ...
    if (isSentFriendRequestsUnavailable(err)) {
      logger.debug(`[friend-sync:${accountId}] getSentFriendRequests unavailable; continuing with accepted friends only`);
      sentRequests = [];
    } else {
      result.errors++;
      logger.warn(`[friend-sync:${accountId}] SDK fetch failed:`, err);
      await logSyncError(orgId, accountId, opts.trigger, err, { phase: 'sdk_fetch_sent_requests' });
      result.durationMs = Date.now() - startedAt;
      return result;
    }
  }
...
function isSentFriendRequestsUnavailable(err: unknown): boolean {
  const anyErr = err as { code?: unknown; message?: unknown };
  return anyErr?.code === 112 || String(anyErr?.message || err).includes('[zalo:112]');
}
