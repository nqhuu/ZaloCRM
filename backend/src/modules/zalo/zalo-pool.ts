  ...
  private async updateAccountDB(
    ...
    try {
      ...
      if (zaloUid !== null) {
        ...
        if (uidOwner) {
          logger.warn(`[zalo:${accountId}] zaloUid ${zaloUid} is already used by account ${uidOwner.id}; status updated without changing zaloUid`);
          effectiveZaloUid = null;
        }
      }
      let updated: { orgId: string };
      try {
        updated = await prisma.zaloAccount.update({
          where: { id: accountId },
          data: {
            status,
            ...(effectiveZaloUid !== null ? { zaloUid: effectiveZaloUid } : {}),
        ...
        if (
          ...
          logger.warn(`[zalo:${accountId}] zaloUid ${zaloUid} is already used by another account; status updated without changing zaloUid`);
          updated = await prisma.zaloAccount.update({
            where: { id: accountId },
            data: {
              status,
