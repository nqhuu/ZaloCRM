...
export async function getOrgDepartmentTree(orgId: string): Promise<DepartmentNode[]> {
  ...
  for (const d of depts) {
    nodeMap.set(d.id, {
      ...
      defaultArchiveRecordType: d.defaultArchiveRecordType,
...
export async function updateDepartment(input: {
  ...
  return await prisma.$transaction(async (tx) => {
    ...
    if (input.defaultArchiveRecordType !== undefined) {
      ...
      if (!allowed.has(input.defaultArchiveRecordType)) throw new Error('Loại hồ sơ mặc định không hợp lệ');
      data.defaultArchiveRecordType = input.defaultArchiveRecordType;
    }
