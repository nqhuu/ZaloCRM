...
export async function userRoutes(app: FastifyInstance) {
  ...
  // POST /api/v1/users/:id/reset-default-password — reset về mật khẩu mặc định.
  app.post('/api/v1/users/:id/reset-default-password', async (request: FastifyRequest, reply: FastifyReply) => {
    const currentUser = request.user!;
    if (!['owner', 'admin'].includes(currentUser.role)) {
      return reply.status(403).send({ error: 'Không có quyền' });
    }
    ...
    if (currentUser.role === 'admin' && target.role === 'owner') {
      return reply.status(403).send({ error: 'Admin không thể reset mật khẩu owner' });
    }
    ...
    logger.info(`Password reset to default for ${target.email} by ${currentUser.email}`);
    return { success: true, defaultPassword: 'Abcd1234' };
  });
